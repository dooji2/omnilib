...
public class OmniFieldListWidget extends ElementListWidget<OmniFieldListWidget.OmniFieldEntry> {
    ...
    public static class OmniFieldEntry extends ElementListWidget.Entry<OmniFieldEntry> {
        ...
        public void render(MatrixStack matrices, int index, int y, int x, int width, int height, int mouseX, int mouseY, boolean hovered, float delta) {
            ...
            DrawableHelper.fill(matrices, x, y, x + this.entryWidth, y + this.entryHeight, bgColor);

            int itemStackX = x + 10;
            int contentStartX = this.itemStack != null ? itemStackX + 20 + 10 : itemStackX;

            if (this.itemStack != null) {
                drawItem(this.itemStack, itemStackX, y + (this.entryHeight - 16) / 2);
            }

            int fieldX = x + this.entryWidth - this.fieldWidth - this.fieldSpacing;

            int textEndX = fieldX - 10;
            drawScrollableText(matrices, MinecraftClient.getInstance().textRenderer, Text.of(this.content), contentStartX, y, textEndX, y + this.entryHeight, 0xFFFFFF);
        ...
        private static void drawScrollableText(MatrixStack matrices, TextRenderer textRenderer, Text text, int startX, int startY, int endX, int endY, int color) {
            ...
            if (textWidth > availableWidth) {
                ...
                DrawableHelper.enableScissor(startX, startY, endX, endY);
                DrawableHelper.drawTextWithShadow(matrices, textRenderer, text, startX - (int) scrollOffset, startY + (endY - startY - textRenderer.fontHeight) / 2, color);
                DrawableHelper.disableScissor();
            } else {
                DrawableHelper.drawTextWithShadow(matrices, textRenderer, text, startX, startY + (endY - startY - textRenderer.fontHeight) / 2, color);
            }
        }
